...
- **10个专业角色**: CEO、CTO、CPO、CMO、CFO、HR、R&D、数据分析师、客户支持、运营专员
...
```go
// 创建 Mailbox 管理器
mailboxManager, _ := mailbox.NewMailboxManager(mailbox.DefaultMailboxManagerConfig())
...
mailboxManager.RegisterMailbox(agents.AgentRoleCEO, handler)
...
| **编排器** | Orchestrator | 集成 MailboxManager 的工作流编排 |
