...
func NewRDAgent() *RDAgent {
	return &RDAgent{
		BaseAgentImpl: NewBaseAgent(
			AgentRoleRD,
			[]string{"功能实现", "测试", "代码审查", "Bug修复", "性能优化"},
			3,
		),
	}
}

func (a *RDAgent) ProcessMessage(msg *Message) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *RDAgent) handleStatusReport(msg *Message) error {
	content := msg.Content
...
func (a *RDAgent) handleTaskAssignment(msg *Message) error {
	content := msg.Content
...
func (a *RDAgent) handleApprovalRequest(msg *Message) error {
	content := msg.Content
...
func (a *RDAgent) GenerateResponse(task *Task) (*Message, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	response := &Message{
		...
		Content: map[string]any{
			...
			"status":            "completed",
			"updated_at":        time.Now(),
			"executive_summary": fmt.Sprintf("Development task '%s' completed by RD", task.Title),
			"code_quality":      "Code reviewed and tested",
		},
