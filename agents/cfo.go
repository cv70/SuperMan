...
func NewCFOAgent() *CFOAgent {
	return &CFOAgent{
		BaseAgentImpl: NewBaseAgent(
			AgentRoleCFO,
			[]string{"财务管理", "预算编制", "财务分析", "成本控制", "风险评估"},
			2,
		),
	}
}

func (a *CFOAgent) ProcessMessage(msg *Message) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *CFOAgent) handleStatusReport(msg *Message) error {
	content := msg.Content
...
func (a *CFOAgent) handleTaskAssignment(msg *Message) error {
	content := msg.Content
...
func (a *CFOAgent) handleDataRequest(msg *Message) error {
	content := msg.Content
...
func (a *CFOAgent) GenerateResponse(task *Task) (*Message, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	response := &Message{
		...
		Content: map[string]any{
			...
			"status":            "completed",
			"updated_at":        time.Now(),
			"executive_summary": fmt.Sprintf("Financial task '%s' completed by CFO", task.Title),
			"financial_impact":  "Budget and financial analysis complete",
		},
