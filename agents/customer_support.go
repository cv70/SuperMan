...
func NewCustomerSupportAgent() *CustomerSupportAgent {
	return &CustomerSupportAgent{
		BaseAgentImpl: NewBaseAgent(
			AgentRoleCustomerSupport,
			[]string{"客户咨询", "问题解决", "反馈收集", "知识库管理", "满意度提升"},
			3,
		),
	}
}

func (a *CustomerSupportAgent) ProcessMessage(msg *Message) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *CustomerSupportAgent) handleStatusReport(msg *Message) error {
	content := msg.Content
...
func (a *CustomerSupportAgent) handleTaskAssignment(msg *Message) error {
	content := msg.Content
...
func (a *CustomerSupportAgent) handleCollaboration(msg *Message) error {
	content := msg.Content
...
func (a *CustomerSupportAgent) GenerateResponse(task *Task) (*Message, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	response := &Message{
		...
		Content: map[string]any{
			...
			"status":            "completed",
			"updated_at":        time.Now(),
			"executive_summary": fmt.Sprintf("Support task '%s' completed by CustomerSupport", task.Title),
			"customer_impact":   "Customer satisfaction improved",
		},
