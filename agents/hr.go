...
func NewHRAgent() *HRAgent {
	return &HRAgent{
		BaseAgentImpl: NewBaseAgent(
			AgentRoleHR,
			[]string{"人才招聘", "绩效评估", "组织发展", "人才管理", "团队优化"},
			2,
		),
	}
}

func (a *HRAgent) ProcessMessage(msg *Message) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *HRAgent) handleStatusReport(msg *Message) error {
	content := msg.Content
...
func (a *HRAgent) handleTaskAssignment(msg *Message) error {
	content := msg.Content
...
func (a *HRAgent) handleCollaboration(msg *Message) error {
	content := msg.Content
...
func (a *HRAgent) GenerateResponse(task *Task) (*Message, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	response := &Message{
		...
		Content: map[string]any{
			...
			"status":              "completed",
			"updated_at":          time.Now(),
			"executive_summary":   fmt.Sprintf("HR task '%s' completed by HR", task.Title),
			"organization_impact": "Team structure and performance optimized",
		},
