...
func NewCPOAgent() *CPOAgent {
	return &CPOAgent{
		BaseAgentImpl: NewBaseAgent(
			AgentRoleCPO,
			[]string{"产品规划", "需求分析", "用户研究", "产品设计", "迭代管理"},
			2,
		),
	}
}

func (a *CPOAgent) ProcessMessage(msg *Message) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *CPOAgent) handleStatusReport(msg *Message) error {
	content := msg.Content
...
func (a *CPOAgent) handleTaskAssignment(msg *Message) error {
	content := msg.Content
...
func (a *CPOAgent) handleDataRequest(msg *Message) error {
	content := msg.Content
...
func (a *CPOAgent) handleAlert(msg *Message) error {
	content := msg.Content
...
func (a *CPOAgent) GenerateResponse(task *Task) (*Message, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	response := &Message{
		...
		Content: map[string]any{
			...
			"status":            "completed",
			"updated_at":        time.Now(),
			"executive_summary": fmt.Sprintf("Product task '%s' completed by CPO", task.Title),
			"product_impact":    "Product roadmap advancement confirmed",
		},
